...
class HarmonicTrialFunction(nn.Module):
    ...
    def gradalpha_forward(self,x):
        return 1/self.alpha -x*x*self.alpha # Need to automatize this with an autograd program. 
    ...
    def score_func_naut4loc(self, x):
        return (1/self.alpha -2*x*x*self.alpha)*(self.alpha * self.alpha + (x * x) * (1.0 - self.alpha ** 4.0)).squeeze(dim=-1)
...
class HydrogenTrialWavefunction(nn.Module):
    ...
    #def local_energy(self, x):
        return (-(1.0/x)-(0.5*autograd_trace_hessian(self.hydro_ansatz_sup,x)/(self.hydro_ansatz_sup(x))))
    def local_energy(self, x):
...
class HeliumTrialWavefunction(nn.Module):
    ...
    def local_energy(self, x):
        return -(self.alpha**2)+(self.alpha)/x[:,0] +(self.alpha)/x[:,1] -(2)/x[:,0] -(2)/x[:,1] + (1)/((x[:,0])**2 +(x[:,1])**2+x[:,0]*x[:,1]*torch.cos(x[:,2] ))
