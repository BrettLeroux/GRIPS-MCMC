...
class NelectronVanderCusp(nn.Module):
    #ansatz given by the Vandermonde determinant of the one electron wavefunctions e^(-alpha_i*r_i) multiplied by product of e^(-1/r_i) for all i
    #input is 1D tensor which determines the number of particles (i.e. the dimension)
    def __init__(self, alpha):
    ...
    def wave(self,x):
        ...
        #input is tensor of size m x alpha.size or m x n x alpha.size
        a = torch.exp(-self.alpha*x.unsqueeze(-1)) - torch.exp(-self.alpha*x.unsqueeze(-2))
        return torch.exp(-torch.sum(1/x, -1)) * torch.prod(a[...,torch.triu(torch.ones(self.alpha.shape[0],self.alpha.shape[0]), diagonal=1).nonzero(as_tuple = True)[0],torch.triu(torch.ones(self.alpha.shape[0],self.alpha.shape[0]), diagonal=1).nonzero(as_tuple = True)[1] ],-1)
...
class NelectronVanderCuspWithMult(nn.Module):
    #ansatz given by the Vandermonde determinant of the one electron wavefunctions e^(-alpha_i * r_i) 
    #multiplied by e^(-beta * (r_1 + r_2 + ... r_N)) and by product of e^(-1/r_i) for all i
    #input is alpha, beta where alpha is 1D tensor which determines the number of particles and beta is scalar
    def __init__(self, alpha, beta):
        super(NelectronVanderWithMult, self).__init__()
        self.alpha = nn.Parameter(alpha)
    ...
    def wave(self,x):
        ...
        #input is tensor of size m x alpha.size or m x n x alpha.size
        a = torch.exp(-self.alpha*x.unsqueeze(-1)) - torch.exp(-self.alpha*x.unsqueeze(-2))
        return torch.exp(-torch.sum(1/x, -1)) * torch.exp(-self.beta * torch.sum(x, -1)) * torch.prod(a[...,torch.triu(torch.ones(self.alpha.shape[0],self.alpha.shape[0]), diagonal=1).nonzero(as_tuple = True)[0],torch.triu(torch.ones(self.alpha.shape[0],self.alpha.shape[0]), diagonal=1).nonzero(as_tuple = True)[1] ],-1) 
