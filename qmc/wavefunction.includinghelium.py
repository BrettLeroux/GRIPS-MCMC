...
class ParticleBoxFunction2(nn.Module):               # Here I have used the gaussian we had discussed. The results are pretty bad.
    def __init__(self, alpha):                       # Here I have removed the normalization but for some reason the results where the 
        super(ParticleBoxFunction2, self).__init__()  # same when I included it. The respective energy is about 2.8 and the expected energy
        self.alpha = nn.Parameter(alpha)              # is actually close to 20. 

    def forward(self, x):
        #
        prod_e_alpha_pi_x = torch.exp(self.alpha*((x-1/2)**2)).prod(dim=-1)
        abs_psi_squared = (prod_e_alpha_pi_x)**2.0
        return torch.log(abs_psi_squared)

    def local_energy(self, x):
        return (self.alpha*(1-2*self.alpha*(x**2-1/2))).sum(dim=-1)    
...
class HeliumTrialWavefunction(nn.Module):
    ...
    def forward(self, x):
        ...
        return 2.0*(3*torch.log((2-self.alpha.clone().detach()))+torch.log(torch.tensor(np.pi))-(2-self.alpha)*(x[0]+x[1]))
